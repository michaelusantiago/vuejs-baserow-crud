<script setup lang="ts">
    ...
    let company = ref<CCompany>(new CCompany('', '', '', '', '', '', null, ''))
    ...
    onMounted(async () => {
        ...
        company.value = await companyStore.getCompany(props.company_id) as ICompany
        isLoading.value = false
        if (refCEO.value) refCEO.value.focus()
    })
    ...
    const submitHandler = async (e: Event) => {
        switch(formSubmitValue.value) {
            ...
            case 1: {
                isSaving.value = true
                error = await companyStore.updateCompany(company.value!)
                isSaving.value = false

                if (!error) {
                    emit('close')
                    emit('refresh')
                } else document.getElementById(error?.field)?.focus()
